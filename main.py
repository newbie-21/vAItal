...
# Page Config
st.set_page_config(page_title="vAItal - AI Health Monitor", page_icon="❤️", layout="wide")
...
# Sidebar
st.sidebar.title("vAItal Controls")
start_btn = st.sidebar.button("Start Monitoring")
stop_btn = st.sidebar.button("Stop Monitoring")

st.sidebar.subheader("Simulate Abnormalities")
sim_hr = st.sidebar.checkbox("Heart Rate")
sim_spo2 = st.sidebar.checkbox("SpO2")
sim_bp = st.sidebar.checkbox("Blood Pressure")
sim_temp = st.sidebar.checkbox("Temperature")
...
auto_send = st.sidebar.checkbox("Auto-send Reports", value=False, help="Automatically send report to doctor when abnormality is detected")
    ...
    if st.session_state.monitoring:
        ...
        if analysis["status"] == "ABNORMAL":
            ...
            if time.time() - st.session_state.last_auto_analysis_time > 10:
                patient_advice, doctor_report, emergency = st.session_state.agent.analyze(vitals, "Auto-detected abnormality. Patient has not provided symptoms yet.")
    ...
    with st.form("symptom_form"):
        ...
        if submit_symptoms:
            if not st.session_state.monitoring:
                ...
                with st.spinner("vAItal is analyzing..."):
